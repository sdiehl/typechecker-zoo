...
The [Effect Rows](./rows.md) chapter covers the row machinery on the effect side, the [Effect Polymorphism](./effect-polymorphism.md) chapter covers what is new compared to the records chapter (the arrow annotation, the value restriction, the top-level closure step), and the [Examples](./examples.md) chapter walks through the integration suite end to end.
